...
:root {
  --nav-shadow: 0 8px 24px rgba(0,0,0,0.06), 0 2px 8px rgba(0,0,0,0.04);
...
:root[data-theme="dark"] {
  --nav-shadow: 0 8px 24px rgba(0,0,0,0.4), 0 2px 8px rgba(0,0,0,0.2);
  --nav-bg: rgba(22, 32, 52, 0.97);
  --bg: #0b1220;
...
/* 移除 background transition，防止 applyTheme() 觸發時出現白色閃爍
   -webkit-backface-visibility 強制 iOS 建立獨立 GPU 合成層              */
html, body {
  height: 100%;
  background: var(--bg);
  color: var(--t1);
...
/* 針對所有會滾動的內容區域增加 padding-bottom，避免內容被懸浮導覽列遮擋 */
...
#nav { 
  ...
  background: var(--nav-bg, rgba(240, 244, 248, 0.97)); 
  ...
  z-index: 90; 
  padding: 0 10px; 
  box-shadow: var(--nav-shadow, 0 8px 24px rgba(0,0,0,0.1)); 
}

/* 滑動背景膠囊：灰色背景整個跟著 active 項目移動
   position:absolute 相對於 #nav（需要 #nav 有 position:relative/fixed）
   left 起點對齊 #nav 的 padding 左邊 */
#nav-indicator {
  position: absolute;
  top: 50%;
  left: 10px;
  ...
  pointer-events: none;
  /* z-index 低於 .ni，確保圖示文字在膠囊上方 */
  z-index: 0;
}

/* 單個導覽列項目 (加上寬高以確保點擊背景呈完美橢圓/圓角矩形) */
.ni { 
  display: flex; 
  ...
  /* z-index 高於 indicator，讓圖示浮在膠囊上方 */
  position: relative;
  z-index: 1;
}
/* 點擊時輕微縮小，給予實體回饋感 */
...
/* Active 狀態：不加自己的背景，完全由 #nav-indicator 統一負責 */
.ni.active { 
...
.hrc-row1 { display: flex; align-items: center; gap: 6px; font-size: 11px; color: var(--t2); font-weight: 600; margin-bottom: 3px; }
...
.hrc-toggle { position: absolute; right: 10px; top: 10px; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; border-radius: 50%; background: hsl(217, 91%, 70%); color: var(--text-blue); font-size: 22px; cursor: pointer; transition: transform 0.3s; }
