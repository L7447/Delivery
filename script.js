...
function updateNavIndicator(activePg) {
  ...
  // 指示條左邊對齊該 .ni 元素左邊（相對於 nav padding 起點）
...
function foldCard(id, e) {
  e.stopPropagation();
  const el = document.getElementById(id); const btn = document.getElementById(id + '-btn'); if (!el || !btn) return;
  if (el.style.maxHeight === '0px' || el.style.maxHeight === '') { el.style.maxHeight = '120px'; btn.style.transform = 'rotate(180deg)'; } 
...
function renderRptOverview() {
  ...
  // 延遲到下一幀：確保 canvas 插入可見 DOM 後再繪製，避免 display:none 時尺寸為 0 導致空白
  if (total > 0 && pieData.length > 0) {
    requestAnimationFrame(() => drawPie('plat-pie', pieLabels, pieData, pieColors));
  }
}
