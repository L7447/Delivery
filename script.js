...
function renderRptOverview() {
  ...
  // 🌟 結構佔比分析卡片（改為柱形圖；全部／各平台按鈕組，按下各平台可顯示單一平台柱形圖）
  html += `
    <div style="display:flex; justify-content:space-between; align-items:center;margin-bottom:4px;">
  ...
  let barLabels = [], barData = [], barColors = [], listHtml = '';
  ...
  if (isAll && total > 0) {
    ...
    barLabels = platData.map(p=>p.name); barData = platData.map(p=>p.val); barColors = platData.map(p=>p.color);

    const platPcts = getExactPercentages(barData);
    ...
    barLabels = ['行程', '獎勵', '小費']; barData = [income, bonus, tips]; barColors = ['#22c55e', '#f59e0b', '#3b82f6'];
  ...
  if (total > 0) {
    html += `
      <div style="position:relative; height:200px; margin-bottom:16px;">
        <canvas id="plat-struct-bar"></canvas>
      </div>
  ...
  if (total > 0 && barData.length > 0) {
    drawStructBar('plat-struct-bar', barLabels, barData, barColors);
  }
}
...
function drawTrendBar(canvasId, labels, datasets, showLegend = true, maxScale = null, horizontal = false) {
  ...
  if (!horizontal && datasets.length > 0) {
    ...
    combinedDatasets.push(lineDataset);
  }
