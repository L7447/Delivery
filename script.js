    ...
    if (S.homeSubTab === 'schedule') {
      if (activePlatforms.length === 0) {
        ...
        activePlatforms.forEach(p => {
          ...
          if (!events) return;
          bottomHtml += `
            <div style="border: 2px solid ${p.color}; background: ${p.color}6; border-radius: 22px; padding: 8px 10px; margin-bottom: 2px;">
              ...
              <div style="display:flex; gap:6px;">${events.map(ev => {
                ...
                return `
                  <div style="flex:1; background: var(--sf); border: 5px solid #009dff; border-radius: 15px; padding: 4px 4px; text-align: center; display:flex; flex-direction:column; justify-content:center;">
