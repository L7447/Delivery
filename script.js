  ...
  document.getElementById('sub-body').innerHTML = `
    <div style="padding:16px;">
      <div style="text-align:center; margin-bottom:24px;">
        ...
        <h2 style="font-size:20px; font-weight:800; color:var(--t1); margin:0;">${v.name}</h2>
        <span style="font-size:12px; font-weight:600; color:var(--blue); background:var(--blue-d); padding:4px 10px; border-radius:12px; display:inline-block; margin-top:6px;">
          ${v.defaultFuel === 'electric' ? '🔋 電動車' : '⛽ ' + v.defaultFuel + '無鉛汽油'}
        </span>
      </div>

      <div class="set-list" style="border:1px solid var(--border);">
        <div class="set-row"><span class="sn" style="color:var(--t3);">車牌號碼</span><span style="font-family:var(--mono); font-weight:800; color:var(--t1);">${v.plate || '未設定'}</span></div>
        <div class="set-row"><span class="sn" style="color:var(--t3);">排氣量</span><span style="font-family:var(--mono); font-weight:800; color:var(--t1);">${v.cc ? v.cc + ' cc' : '未設定'}</span></div>
        <div class="set-row"><span class="sn" style="color:var(--t3);">車輛顏色</span><span style="font-weight:800; color:var(--t1);">${v.color || '未設定'}</span></div>
...
function openPrivacyPolicy(fromRegister = false) {
  ...
  document.getElementById('privacy-body').innerHTML = `
    <div style="font-size:13px; color:var(--t1); line-height:1.8; padding:8px 4px;">
      <div style="color:var(--acc); font-size:16px; font-weight:500;">1. 我們使用到的資料</div>
      • 帳號（ 電子郵件 ）註冊驗證使用。<br><br>

      <div style="color:var(--acc); font-size:16px; font-weight:500;">2. 我們如何使用資訊</div>
      • 統計「 使用人數 」。<br><br>

      <div style="color:var(--acc); font-size:16px; font-weight:500;">3. 資料及使用安全</div>
      ...
      • 我們已將密碼雜湊演算法從單純的 SHA-256 升級為 PBKDF2。<br>
        <div style="color:var(--text-red); font-weight:500;">&emsp;&emsp;🚩「 PBKDF2 」的主要優勢：</div>

        &emsp;🔶加入隨機鹽值（Salting）：有效抵抗彩虹表（Rainbow Table）攻擊。<br>
        <div style="color:var(--text-blue);">&emsp;&emsp;🔷高迭代次數（Key Stretching）：進行 100,000 次反覆運算，大幅增加暴力破解所需時間。</div>
        &emsp;🔶計算密集型設計：顯著提升對 GPU/ASIC 大規模破解的抵抗力。<br>

        <div style="color:var(--text-blue);">&emsp;&emsp;🏁相較於單純使用 SHA-256；PBKDF2 能提供更強的防護能力，有效降低密碼在資料外洩時被快速破解的風險。</div><br>

      <div style="color:var(--acc); font-size:16px; font-weight:500;">4. 您的權利</div>
      • 您可隨時聯繫我們，幫您刪除帳號。<br><br>

      <div style="color:var(--acc); font-size:16px; font-weight:500;">5. 聯絡方式</div>
...
function init() {
  ...
  // 所有 DOM 穩定後，下一幀恢復 transition，並初始化導覽列指示條
